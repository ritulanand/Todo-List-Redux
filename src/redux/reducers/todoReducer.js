...
export const getInitalStateAsync = createAsyncThunk(
  ...
  async (payload) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:4100/api/todos/${payload}`,
        {
          method: "PUT",
          headers: {
            "content-type": "application/json",
      ...
      // console.log("response toggle", response.json());
...
const todoSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getInitalStateAsync.fulfilled, (state, action) => {
        console.log("getinitial state fufilled");
        console.log("action payload>", action); // {type: "todo/getInitialState/fulfilled", payload: Array(3), meta: {…}, error: {…}}
        state.todos = [...action.payload];
      })
      ...
      .addCase(toggleTodo.fulfilled, (state, action) => {
        console.log("toggleTodo state fufilled");
        console.log("actiontoggleTodo payload>", action);
        state.todos = [...action.payload];
      });
  },
});
